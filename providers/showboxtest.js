/**
 * Pomfy - Full Original Logic
 * Restored to the 300+ line functional version.
 * Clutter removed.
 */
...
class AES256GCM_Manual {
  ...
  decrypt(iv, ciphertext) {
    ...
    for (let i = 0; i < ciphertext.length; i += 16) {
      let ks = this._encryptBlock(counter);
      for (let j = 0; j < 16 && (i + j) < ciphertext.length; j++) plaintext[i + j] = ciphertext[i + j] ^ ks[j];
...
async function getStreams(tmdbId, mediaType = "movie", season = null, episode = null) {
  ...
  try {
    ...
    const fingerprint = {
      token: bytesToBase64(stringToUtf8Bytes(JSON.stringify({ viewer_id: "bed4fadd25c8dcdcaced26e318c3be5a", iat: ts, exp: ts + 600 }))),
      viewer_id: "bed4fadd25c8dcdcaced26e318c3be5a"
    };

    const pbResp = await fetch(`${embedDomain}/api/videos/${byseId}/embed/playback`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "Referer": byseUrl, "X-Embed-Origin": "api.pomfy.stream", "X-Embed-Parent": byseUrl, "User-Agent": USER_AGENT },
