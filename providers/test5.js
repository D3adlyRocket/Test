// movies4u.js
// Fixed Nuvio-compatible Movies4u provider with Enhanced Quality Mapping
  ...
  try {
    ...
    for (const item of directWatchLinks) {
      ...
      if (directM3u8) {
        ...
        // Detect real quality from m3u8 playlist
        const detectedQuality = await detectM3U8Quality(directM3u8, { Referer: "https://m4uplay.store/" });

        // Decide final quality
        const finalQuality = (extractQuality(item.text + " " + item.href + " " + siteTitleContext) !== "Unknown")
          ? extractQuality(item.text + " " + item.href + " " + siteTitleContext)
          : (urlMeta.quality !== "Unknown" ? urlMeta.quality : (detectedQuality || "1080p"));
        ...
        rawStreamsList.push({
          ...
          meta: { ...parseExtraMetadata(item.text + " " + item.href + " " + siteTitleContext), size: detectedSize || urlMeta.meta.size },
          url: directM3u8,
          ...
          for (const target of targetUrls) {
            ...
            if (target.href.includes("m4uplay.store")) {
              ...
              if (directM3u8) {
                const urlMeta = extractMetadataFromUrl(directM3u8);
                const detectedQuality = await detectM3U8Quality(directM3u8, { Referer: "https://m4uplay.store/" });
                const finalQuality = (quality !== "Unknown") ? quality : (urlMeta.quality !== "Unknown" ? urlMeta.quality : (detectedQuality || "1080p"));
                const detectedSize = await detectFileSize(directM3u8, { Referer: "https://m4uplay.store/" });
                rawStreamsList.push({
                  server: "M4U Player",
                  quality: finalQuality,
                  meta: { ...meta, ...urlMeta.meta, size: detectedSize || urlMeta.meta.size },
                  url: directM3u8,
                  headers: { Referer: "https://m4uplay.store/", Origin: "https://m4uplay.store", "User-Agent": HEADERS["User-Agent"] }
                });
              }
              ...
              for (const linkItem of extractedLinks) {
                ...
                rawStreamsList.push({
                  ...
                  meta: {
                    ...
                    size: detectedSize || innerMeta.size !== "N/A" ? innerMeta.size : (urlMeta.meta.size !== "N/A" ? urlMeta.meta.size : "1.4GB"),
    ...
    // Sort streams by quality weight
