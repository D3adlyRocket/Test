...
// TV OPTIMIZATION: Updated to Sony Bravia User-Agent
var HEADERS = {
    ...
    for (const domain of KNOWN_DOMAINS) {
      try {
        const res = yield fetch(`${domain}/`, {
          method: "HEAD",
          headers: HEADERS,
          signal: AbortSignal.timeout(8e3) // Increased slightly for TV hardware
        });
...
function fetchWithResponse(_0) {
  return __async(this, arguments, function* (url, options = {}) {
    const response = yield fetch(url, __spreadProps(__spreadValues({}, options), {
      ...
      signal: AbortSignal.timeout(15e3) // TV FIX: Hard timeout for network stability
    }));
    if (!response.ok) {
...
function getStreams(tmdbId, type, season, episode) {
  return __async(this, null, function* () {
    try {
      ...
      if (stream) {
        // TV FIX: Enhanced return object for TV playback compatibility
        return [{
          name: "Dizipal TV",
          url: stream.url,
          quality: stream.quality || "Auto",
          headers: __spreadValues(stream.headers || {}, { "User-Agent": HEADERS["User-Agent"] })
        }];
      }
    } catch (e) {
  ...
  return __async(this, null, function* () {
    try {
      ...
      if (seriesSlug) {
        ...
        try {
          const testRes = yield fetch(guessUrl, { method: "HEAD", headers: HEADERS, signal: AbortSignal.timeout(5e3) });
          if (testRes.ok)
            return guessUrl;
        } catch (_) {
        }
