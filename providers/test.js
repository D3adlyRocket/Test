...
function getTmdbMeta(mediaType, tmdbId, season) {
  return __async(this, null, function* () {
    ...
    var duration = data.runtime ? `${data.runtime}m` : "N/A";
    if (mediaType === "movie") {
      title = data.title;
    ...
    return { title, year, imdbId, isAnime, originalTitle: data.original_name || data.original_title || "", seasonName, seasonEpisodeCount, duration };
  });
}
...
function formatRegularStreams(data, meta, mediaType, seasonId, episodeId) {
  ...
  streams.sort(function(a, b) {
    return streamRank(b) - streamRank(a);
  });
  return streams;
}
    ...
    try {
      ...
      if (primaryItem) {
        ...
        if (primaryData && primaryData.sources && primaryData.sources.length > 0) {
          ...
          var primaryStreams = formatRegularStreams(primaryData, meta, mType, seasonId, episodeId);
      ...
      var streams = formatRegularStreams(data, meta, mType, seasonId, episodeId);
