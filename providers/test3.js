...
// DYNAMIC DOMAIN RESOLVER (From Script 1)
function getLatestDomain() {
  var now = Date.now();
  if (now - domainCache.ts < 36e5) {
    return Promise.resolve(domainCache.url);
  }
  return fetch(DOMAINS_URL, { headers: { "User-Agent": USER_AGENT } })
...
function getBaseUrl(url) {
  if (!url) return domainCache.url;
  var match = url.match(/^(https?:\/\/[^\/]+)/);
  return match ? match[1] : domainCache.url;
}
...
function searchByTitle(title, year, domain) {
...
function getStreams(tmdbId, mediaType, season, episode) {
  ...
  var allStreams = [];
  var activeDomain = BASE_URL;

  // STEP 1: Always grab the working fallback domain dynamically first
  return getLatestDomain().then(function(latestDomain) {
    ...
    // STEP 2: Use the dynamic target domain for finding posts
    return searchByTitle(tmdbDetails.title, tmdbDetails.year, activeDomain);
    ...
    function processResult(index) {
      ...
      return linksPromise.then(function(links) {
        var extractPromises = links.map(function(linkData) {
          ...
          var finalLinkPromise = sourceLink.indexOf("unblockedgames") !== -1 || sourceLink.indexOf("sid=") !== -1 ? bypassHrefli(sourceLink) : Promise.resolve(sourceLink);
