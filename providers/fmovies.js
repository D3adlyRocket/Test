    ...
    for (const domain of KNOWN_DOMAINS) {
      try {
        ...
        if (res.ok || res.status === 302 || res.status === 301) {
          ...
          console.log(`[Dizipal] Active domain: ${finalUrl}`);
          return finalUrl;
        }
      } catch (_) {
      }
...
function getTmdbTitle(tmdbId, mediaType) {
  return __async(this, null, function* () {
    ...
    if (htmlResult)
      return htmlResult;
    console.log(`${PROVIDER_TAG} HTML scraping failed, trying TMDB REST API...`);
    const apiResult = yield getTmdbTitleFromApi(tmdbId, mediaType);
    if (apiResult)
      return apiResult;
    console.error(`${PROVIDER_TAG} Both methods failed: TMDB ID=${tmdbId}`);
...
function resolveDizipal(url, activeUrl) {
  return __async(this, null, function* () {
    try {
      ...
      console.log(`${PROVIDER_TAG2} Resolving: ${url}`);
      ...
      if (configToken) {
        console.log(`${PROVIDER_TAG2} Config token found: ${configToken.substring(0, 20)}...`);
      ...
      if (embedUrl) {
        console.log(`${PROVIDER_TAG2} Direct embed: ${embedUrl}`);
  ...
  return __async(this, null, function* () {
    ...
    try {
      ...
      try {
        ...
        console.error(`${PROVIDER_TAG2} Config JSON parse error: ${e.message}`);
        return null;
      }
