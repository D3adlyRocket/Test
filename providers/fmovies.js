// providers/uhdmovies.js — converted to ES Module
...
function cleanTitle(title) {
  ...
  for (var i = 0; i < parts.length; i++) {
    ...
    for (var q = 0; q < qualityTags.length; q++) {
      if (p.indexOf(qualityTags[q].toLowerCase()) !== -1) { startIndex = i; break; }
    }
  ...
  for (var j = parts.length - 1; j >= 0; j--) {
    ...
    for (var t = 0; t < allTags.length; t++) {
      if (pp.indexOf(allTags[t].toLowerCase()) !== -1) { found = true; break; }
    }
    if (found) { endIndex = j; break; }
  }
  if (startIndex !== -1 && endIndex !== -1 && endIndex >= startIndex) return parts.slice(startIndex, endIndex + 1).join(".");
  else if (startIndex !== -1) return parts.slice(startIndex).join(".");
  return parts.slice(-3).join(".");
}
function fetchText(url, extraHeaders) {
  var headers = Object.assign({ "User-Agent": USER_AGENT }, extraHeaders || {});
  return fetch(url, { headers, redirect: "follow" }).then(function(res) { return res.text(); });
}
function fetchJson(url) {
  return fetch(url, { headers: { "User-Agent": USER_AGENT } }).then(function(res) { return res.json(); });
}
function getTmdbDetails(tmdbId, mediaType) {
  ...
  return fetchJson(url).then(function(data) {
    if (isSeries) return { title: data.name, year: data.first_air_date ? data.first_air_date.slice(0, 4) : null };
    return { title: data.title, year: data.release_date ? data.release_date.slice(0, 4) : null };
  }).catch(function(err) {
...
function searchByTitle(title, year) {
  ...
  return fetchText(url).then(function(html) { return parseSearchResults(html); }).catch(function(err) {
...
function parseSearchResults(html) {
  ...
  for (var i = 1; i < chunks.length; i++) {
    ...
    if (href && title) results.push({ title, url: href, rawTitle: titleRaw });
  }
  return results;
}
function bypassHrefli(url) {
  ...
  return fetchText(url).then(function(html) {
    ...
    return fetch(formUrl, {
      ...
      headers: { "User-Agent": USER_AGENT, "Content-Type": "application/x-www-form-urlencoded" },
      body: toFormEncoded(formData)
    }).then(function(res) { return res.text().then(function(t) { return { html: t, formData }; }); });
    ...
    return fetchText(host + "?go=" + skToken, { "Cookie": skToken + "=" + wpHttp2 });
  }).then(function(html) {
    if (!html) return null;
    return extractMetaRefresh(html) || null;
...
function extractResumeBot(url) {
  return fetchText(url).then(function(html) {
    ...
    return fetch(baseUrl + "/download?id=" + path, {
      method: "POST",
      headers: { "User-Agent": USER_AGENT, "Content-Type": "application/x-www-form-urlencoded", "Accept": "*/*", "Origin": baseUrl, "Referer": url },
  ...
  return fetchText(pageUrl).then(function(html) {
    ...
    for (var i = 0; i < parts.length; i++) {
      ...
      for (var j = i + 1; j < Math.min(i + 6, parts.length); j++) {
        ...
        if (btnM) { links.push({ sourceName, sourceLink: btnM[1] }); break; }
      }
    }
    return links;
  }).catch(function(err) {
      ...
      if (hasEpisodeLink) {
        ...
        if (currentSeason === targetSeason) {
          ...
          if (targetEpisode <= episodeLinks.length && targetEpisode >= 1) {
            ...
            links.push({ sourceLink: link, quality: buildQualityLabel(prevDetails), size: sizeM ? sizeM[1] : null, details: prevDetails });
          }
        }
        currentSeason++;
      }
...
export async function getStreams(tmdbId, mediaType, season, episode) {
  ...
  return getTmdbDetails(tmdbId, mediaType).then(function(tmdbDetails) {
    ...
    function processResult(index) {
      ...
      var linksPromise = isSeries && season && episode
        ? getTvEpisodeLink(result.url, season, episode)
        : getMovieLinks(result.url);
      return linksPromise.then(function(links) {
        var extractPromises = links.map(function(linkData) {
          ...
          var finalLinkPromise = sourceLink.indexOf("unblockedgames") !== -1
            ? bypassHrefli(sourceLink)
            : Promise.resolve(sourceLink);
        ...
        return Promise.all(extractPromises).then(function(results) {
          results.forEach(function(s) { allStreams = allStreams.concat(s); });
    ...
    return processResult(0).then(function(streams) {
      function scoreStream(s) {
        var q = s.quality || "";
        var rScore = /^4K/i.test(q) ? 4 : /1080p/i.test(q) ? 3 : /720p/i.test(q) ? 2 : /480p/i.test(q) ? 1 : 0;
        var sScore = /remux/i.test(q) ? 5 : /blu.?ray/i.test(q) ? 4 : /web.?dl/i.test(q) ? 3 : /webrip/i.test(q) ? 2 : /hdrip|dvdrip|hdtv/i.test(q) ? 1 : 0;
