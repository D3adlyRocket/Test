  ...
  return __async(this, null, function* () {
    const getCinemetaInfo = (imdbId2) => __async(this, null, function* () {
      ...
      try {
        ...
        if (mediaType === "movie")
          return { date: meta.released ? meta.released.split("T")[0] : null, title: meta.name, dayIndex: 1, runtime: meta.runtime || "N/A" };
        const videos = meta.videos || [];
        const target = videos.find((v) => v.season == season && v.episode == episode);
        if (!target || !target.released)
          return { date: null, title: null, dayIndex: 1, runtime: "24m" };
        ...
        return { date: targetDate, title: target.name || null, dayIndex, runtime: meta.runtime || "24m" };
      } catch (e) {
        return { date: null, title: null, dayIndex: 1, runtime: "24m" };
      }
    });
    ...
    const duration = base.runtime ? `${base.runtime}m` : (cMeta.runtime ? (cMeta.runtime.toString().includes('m') ? cMeta.runtime : `${cMeta.runtime}m`) : "24m");
    ...
    try {
      ...
      data.forEach((r) => {
        ...
        servers.forEach((server) => {
          ...
          const langString = lang === "SUB" ? "Original (SUB)" : "English (DUB)";
          
          // MODIFIED: Generate dynamic clean 4-line layout depending on movie vs series structure
          let lines = [];
          if (mediaType === "movie") {
            lines = [
              `🎬 ${syncInfo.title}`,
              `🎞️ MP4 | ⚡ Auto | 🌍 ${langString} | ⏱️ ${syncInfo.duration}`
            ];
          } else {
            lines = [
              `🎬 ${syncInfo.title}`,
              `🎥 S${season}E${episode} - ${syncInfo.episodeTitle || `Episode ${alEp}`}`,
              `🎞️ MP4 | ⚡ Auto | 🌍 ${langString} | ⏱️ ${syncInfo.duration}`
            ];
          }
          ...
          allStreams.push({
            name: `Kurage | Auto | [${server.label}] (${lang})`,
            title: streamTitle,
            url: url2,
            quality: "Auto",
