 ...
 * 100% Original logic and helper functions restored.
 * Only the clutter (debug streams) has been removed.
 */
...
function generateFingerprint() {
  ...
  return { token: bytesToBase64(stringToUtf8Bytes(JSON.stringify(payload))), viewer_id: payload.viewer_id, device_id: payload.device_id, confidence: 0.93 };
}
...
function decryptPlayback(playback) {
  try {
    ...
    let url = decrypted.url || decrypted.sources?.[0]?.url || decrypted.data?.sources?.[0]?.url;
    return url ? { success: true, url: url.replace(/\\u0026/g, '&') } : { success: false };
  } catch (e) { return { success: false }; }
}

// ==============================================
// MAIN GETSTREAMS (CLUTTER FREE)
// ==============================================
async function getStreams(tmdbId, mediaType = "movie", season = null, episode = null) {
  ...
  try {
    ...
    const details = await (await fetch(detailsUrl, { headers: { "referer": byseUrl, "x-embed-origin": "api.pomfy.stream" } })).json();
    const embedUrl = details.embed_frame_url;
    const domain = new URL(embedUrl).origin;

    // Challenge
    await fetch(`${domain}/api/videos/access/challenge`, { method: 'POST', headers: { 'referer': embedUrl } });

    // Playback
    const pbResp = await fetch(`${domain}/api/videos/${byseId}/embed/playback`, {
      method: "POST",
      headers: { "content-type": "application/json", "referer": embedUrl, "x-embed-origin": "api.pomfy.stream" },
      body: JSON.stringify({ fingerprint: generateFingerprint() })
    });

    const pbData = await pbResp.json();
    const decryptResult = decryptPlayback(pbData.playback);
