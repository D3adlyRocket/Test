...
let MAIN_URL = "https://moviesdrives.cv";
    ...
    try {
        ...
        for (const a of anchors) {
            ...
            /* INDEX LINKS */
                ...
                const indexPage = await fetch(`https://new17.gdflix.net/file${href}`).then(r => r.text());
                const $$ = cheerio.load(indexPage);

                const btns = $$('a.btn-outline-info').get();
                for (const b of btns) {
                    const serverUrl = 'https://new17.gdflix.net/file' + $$(b).attr('href');
