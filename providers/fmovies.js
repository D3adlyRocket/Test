...
async function detectQuality(m3u8Url) {  
    try {  
        const res = await fetch(m3u8Url, { headers: { "User-Agent": USER_AGENT, "Referer": "https://pomfy.online/" } });  
        if (!res.ok) return "Auto";  
        const text = await res.text();  
        if (text.includes("RESOLUTION=1920x1080") || text.includes("BANDWIDTH=8000000")) return "1080p";  
        if (text.includes("RESOLUTION=1280x720") || text.includes("BANDWIDTH=4000000")) return "720p";  
...
async function getM3U8Size(m3u8Url, durationText) {  
    try {  
        ...
        const segments = lines.filter(line => line && !line.startsWith("#") && (line.includes(".ts") || line.includes(".m4s")));  
        ...
        if (sampleSegments.length > 0) {  
            ...
            for (const seg of sampleSegments) {  
                ...
                try {  
                    const head = await fetch(segUrl, { method: "HEAD", headers: { "User-Agent": USER_AGENT } });  
...
async function getStreams(tmdbId, mediaType = "movie", season = null, episode = null) {  
    ...
    try {  
        ...
        let linkMatch = html.match(/const\s+link\s*=\s*"([^"]+)"/) || html.match(/"link"\s*:\s*"([^"]+)"/);  
        if (!linkMatch) return [];  
        ...
        const detailsResponse = await fetch(`https://pomfy-cdn.shop/api/videos/${byseId}/embed/details`, {  
            headers: { "accept": "*/*", "referer": byseUrl, "x-embed-origin": "api.pomfy.stream", "user-agent": USER_AGENT, "Cookie": COOKIE }  
        });  
        ...
        if (decryptResult.success) {  
            ...
            const resLabel = await detectQuality(decryptResult.url);  
            const language = detailsData.language || "English • Portuguese";  
            const size = await getM3U8Size(decryptResult.url, meta.duration);  
            streams.push({  
                name: '🎦',   
                title: buildTitle(meta, resLabel, language, 'm3u8', size, 'Direct', mediaType === "tv" ? s : null, mediaType === "tv" ? e : null),  
