...
var DEFAULT_MAIN_URL = "https://4khdhub.fans"; // Updated to currently active domain
...
// FIX: Updated Redirect Logic for Modern HubCDN/Series formats
function getRedirectLinks(url) {
  return __async(this, null, function* () {
    ...
    // Handle the hash at the end of newer HubCDN links
    if (!combined && url.includes("id=")) {
      combined = url.split('id=')[1].split('::')[0];
    }
    ...
    try {
      ...
      return decoded || decodeBase64(combined);
    } catch (error) {
...
// FIX: Updated Episode Collector to handle "Series" archive style pages
function collectEpisodeLinks($, pageUrl, season, episode) {
  const directEpisodeLinks = [];
  const sNum = parseInt(season, 10);
  const eNum = parseInt(episode, 10);

  $("div.episodes-list div.season-item, div.download-item").each((_, seasonEl) => {
    const fullText = $(seasonEl).text().toLowerCase();
    const isTargetSeason = fullText.includes(`season ${sNum}`) || 
                           fullText.includes(`s${sNum.toString().padStart(2, '0')}`) ||
                           fullText.includes(`s${sNum} `);
                           
    if (!isTargetSeason) return;

    $(seasonEl).find("a[href]").each((__, linkEl) => {
      const href = fixUrl($(linkEl).attr("href"), pageUrl);
      const linkText = $(linkEl).text().toLowerCase() + " " + $(seasonEl).text().toLowerCase();
      
      const isTargetEpisode = linkText.includes(`episode ${eNum}`) || 
                              linkText.includes(`e${eNum.toString().padStart(2, '0')}`) ||
                              linkText.includes(`e${eNum} `) ||
                              linkText.includes(`ep ${eNum}`);

      if (isTargetEpisode) {
        directEpisodeLinks.push({ 
            url: href, 
            label: $(linkEl).text().trim() || `S${season}E${episode}`, 
            rawHtml: $(seasonEl).html() 
        });
      }
    });
  });
  return directEpisodeLinks;
}
...
function extractStreams(tmdbId, mediaType, season, episode) {
  return __async(this, null, function* () {
    ...
    // FIX: Optimized "Movie Page" check for series archive links
    const isMoviePage = mediaType === "movie" || ($("div.episodes-list").length === 0 && !html.toLowerCase().includes("season"));
    
    let links = isMoviePage 
