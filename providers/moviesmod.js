/** * Pomfy - Surgical Fix (2026 Edition)  
 */  
...
const TARGET_ORIGIN = "https://398fitus.com"; 
...
function generateFingerprint() {  
  ...
  return { token: bytesToBase64(stringToUtf8Bytes(JSON.stringify(payload))) };  
}  
...
async function getStreams(tmdbId, mediaType = "movie", season = null, episode = null) {  
  ...
  try {  
    ...
    // Look for the embed link in the page
    const linkMatch = html.match(/const\s+link\s*=\s*["']([^"']+)["']/);  
    if (!linkMatch) return [];  

    const byseUrl = linkMatch[1];  
    const byseId = byseUrl.split("/").pop();  
    
    // FETCH DETAILS (Using the domain from your screenshot)
    ...
    // FETCH PLAYBACK (Decryption step)
    ...
    // Decrypt the payload
    const iv = base64ToBytes(playbackData.playback.iv);  
    const k1 = base64ToBytes(playbackData.playback.key_parts[0]);  
    const k2 = base64ToBytes(playbackData.playback.key_parts[1]);  
    const key = new Uint8Array([...k1, ...k2]);  
    const encrypted = base64ToBytes(playbackData.playback.payload);  
    ...
    if (m3u8) {
        streams.push({
            name: "Pomfy",
            quality: detailsData.language || "Multi-Audio",
            url: m3u8.replace(/\\u0026/g, '&'),
            headers: {
                "User-Agent": USER_AGENT,
                "Referer": embedUrl,
